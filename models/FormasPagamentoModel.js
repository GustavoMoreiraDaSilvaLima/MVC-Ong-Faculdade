...
class formasPagamentoModel {
    ...
    get pagamento_id() { return this.#pagamento_id }
    set pagamento_id(id) { this.#pagamento_id = id }

    get pagamento_nome() { return this.#pagamento_nome }
    set pagamento_nome(nome) { this.#pagamento_nome = nome }
    ...
    async listar() {
        ...
        for (let i = 0; i < rows.length; i++) {
            lista.push(new formasPagamentoModel(rows[i]["pagamento_id"], rows[i]["pagamento_nome"], rows[i]["pagamento_tipo"]));
        }
        return lista;
