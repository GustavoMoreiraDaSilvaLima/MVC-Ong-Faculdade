...
class eventosModel {
    ...
    constructor(codigo, nome, desc, inicio, dia, duracao, local, imagem) {
    ...
    async obterEvento(id) {
        ...
        if (rows.length > 0) {
            ...
            evento = new eventosModel(row["evento_id"], row["evento_nome"], row["evento_descricao"], row["evento_inicio"], row["evento_data"], row["evento_duracao"], row["evento_local"], imagem);
    ...
    async exibirEvento() {
        ...
        if (rows.length > 0) {
            for (let i = 0; i < rows.length; i++) {
                ...
                listaRetorno.push(new eventosModel(row["evento_id"], row["evento_nome"], row["evento_descricao"], row["evento_inicio"], row["evento_data"], row["evento_duracao"], row["evento_local"], imagem));
            }
        }
        return listaRetorno;
    }
    ...
    async inclu_alterar_Evento() {
        if (this.evento_id == 0) {
            ...
            let sql = "update tb_evento set evento_nome = ?, evento_descricao = ?, evento_local = ?, evento_inicio = ?, evento_data = ?, evento_duracao = ?, evento_imagem = ? where evento_id = ?"
            let valores = [this.#evento_nome, this.#evento_descricao, this.#evento_local, this.#evento_inicio, this.#evento_data, this.#evento_duracao, this.#evento_imagem, this.#evento_id];
