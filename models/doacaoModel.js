...
class doacaocaoModel {
  ...
  get doacao_tipo() {
    return this.#doacao_tipo;
  }
  set doacao_tipo(tipo) {
    this.#doacao_tipo = tipo;
  }

  set doacao_usuario(id) {
    this.#doacao_usuario = id;
  }
  get doacao_usuario() {
    return this.#doacao_usuario;
  }

  get doacao_data() {
    return this.#doacao_data;
  }
  set doacao_data(data) {
    this.#doacao_data = data;
  }

  get doacao_nome() {
    return this.#doacao_nome;
  }
  set doacao_nome(nome) {
    this.#doacao_nome = nome;
  }

  get doacao_valor() {
    return this.#doacao_valor;
  }
  set doacao_valor(valor) {
    this.#doacao_valor = valor;
  }

  get doacao_status() {
    return this.#doacao_status;
  }
  set doacao_status(status) {
  ...
  async doacao_inserir_atualizar() {
    if (this.#doacao_id == 0) {
      let sql =
        "insert into tb_doacao (doacao_id,doacao_tipo,doacao_usuario,doacao_status,doacao_nome,doacao_valor,doacao_data) values (?,?,?,?,?,?,?)";
      ...
      let sql =
        "update tb_doacao set doacao_status = ?, doacao_nome = ?, doacao_tipo = ?,doacao_valor = ?, doacao_usuario = ? where doacao_id = ?";
      let valores = [
        this.#doacao_status,
        this.#doacao_nome,
        this.#doacao_tipo,
