...
class LoginModel {
  ...
  get login_senha() {
    return this.#login_senha;
  }
  // Setters
  set login_id(id) {
    this.#login_id = id;
  }
  set login_perfil(perfil) {
    this.#login_perfil = perfil;
  }
  set login_usuario(usuario) {
    this.#login_usuario = usuario;
  }
  ...
  async obterPorEmailSenha(email, senha) {
    let sql =
      "select * from tb_login where login_email = ? and login_senha = ?";
