...
class ProdutoModel {
  ...
  async gravar() {
    if (this.#produtoId == 0) {
      let sql =
        "insert into tb_produto (prd_cod, prd_nome, prd_quantidade, cat_id, mar_id, prd_imagem, prd_valor) values (?, ?, ?, ?, ?, ?, ?)";
      ...
      //alterar
      let sql =
        "update tb_produto set prd_cod = ?, prd_nome =?, prd_quantidade= ?, cat_id = ?, mar_id = ?, prd_imagem = ?, prd_valor = ? where prd_id = ?";
      ...
      if (rows.length > 0) {
        for (let i = 0; i < rows.length; i++) {
          ...
          let imagem = row["prd_imagem"]
            ? global.CAMINHO_IMG_BROWSER + row["prd_imagem"]
            : global.CAMINHO_IMG_BROWSER + "sem-foto.png";
