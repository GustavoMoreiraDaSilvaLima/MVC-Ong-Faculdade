...
class HomeController {
  ...
  async especNoticia(req, res) {
    if (
      (contador_esp_noticias == 0 || contador_esp_noticias != req.params.id) &&
      req.params.id > 0
    ) {
  ...
  DoaCartaoView(req, res) {
    res.render("formas_pagamento/cartao", {
      layout: "formas_pagamento/cartao",
    });
  }
  DoaBoletoView(req, res) {
    res.render("formas_pagamento/boleto", {
      layout: "formas_pagamento/boleto",
    });
  }
  DoaPixView(req, res) {
