document.addEventListener("DOMContentLoaded", function () {
  ...
  function limparValidacao() {
    ...
    document.getElementById("eventDescription").style["border-color"] =
      "#ced4da";
  ...
  function AlterarPagina() {
    ...
    function CarregarBotao() {
      let btnAlterarPatrimonio = document.getElementById(
        "btnAlterarPatrimonio"
      );
      let btnAlterarProduto = document.getElementById("btnAlterarProduto");
      let btnAdicionar = document.querySelectorAll(".btnAdicionar");
      let btnAdicionarPatrimonio = document.querySelectorAll(
        ".btnAdicionarPatrimonio"
      );
      ...
      for (let i = 0; i < btnAdicionarPatrimonio.length; i++) {
        btnAdicionarPatrimonio[i].addEventListener(
          "click",
          AdicionarItemPatrimonio
        );
      }
        ...
        html += `
            <tr data-idpatrimonioEnviar=${
              ListaPatrimonio.item[posicaoPatrimonio[i]].patrimonioid
                ...
                <td data-quantidade="${
                  ListaPatrimonio.item[posicaoPatrimonio[i]]
                    .patrimonioQuantidade
                }">${
          ListaPatrimonio.item[posicaoPatrimonio[i]].patrimonioQuantidade
        }</td>
                <td><input class="form-control quantidade" type="number" placeholder="Coloque a quantidade" value="1"></td>
                <td><button data-codigoremover="${
                  ListaPatrimonio.item[posicaoPatrimonio[i]].patrimonioid
                }" data-codigoposicao="${
          posicaoPatrimonio[i]
        }" class="btn btn-danger btnExcluir"><i class="fas fa-trash"></i></td>
            </tr>       
            `;
      }
      html += `
    ...
    function CarregarListaProduto() {
      ...
      for (let i = 0; i < posicao.length; i++) {
        html += `
            <tr data-idprodutoEnviar=${
              ListaProdutos.item[posicao[i]].produtoId
            }>
                <td><img src="${
                  ListaProdutos.item[posicao[i]].produtoImagem
                }" width="80"></td>
                <td>${ListaProdutos.item[posicao[i]].produtoNome}</td>
                <td data-quantidade="${
                  ListaProdutos.item[posicao[i]].produtoQuantidade
                }">${ListaProdutos.item[posicao[i]].produtoQuantidade}</td>
                <td><input class="form-control quantidade" type="number" placeholder="Coloque a quantidade" value="1"></td>
                <td><button data-codigoremover="${
                  ListaProdutos.item[posicao[i]].produtoId
                }" data-codigoposicao="${
    ...
    function VerificaValor() {
      ...
      let ValorMaximo =
        this.parentElement.previousElementSibling.dataset.quantidade;
        ...
        .then(async (r) => {
          if (r.ok) {
            ...
            if (SaidaCompleta.length == 2) {
              ...
              let btnAlterarProduto =
                document.getElementById("btnAlterarProduto");
